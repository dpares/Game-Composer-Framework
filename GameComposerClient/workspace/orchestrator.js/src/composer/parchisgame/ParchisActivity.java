 ...
 */
public class ParchisActivity extends ComposerGameActivity {
    ...
    public JSONObject getCommonDataJSON() {
        try {
            ...
            throw new ComposerGameException("Error parsing commonData JSON", e);
        }
    }
    ...
    public void update(JSONArray players, JSONObject commonData) {
        try {
            ...
            this.playersList = new ArrayList<ComposerPlayer>();
            ...
            throw new ComposerGameException("Error parsing JSON in update", e);
        }
    }
            ...
            handler.postDelayed(new Runnable() {
                public void run() {
                    try {
                        ...
                        throw new ComposerGameException("Error waiting for die roll", e);
                    }
                }
            }, 10);
        }
    ...
    public void showNextMove(View v) {
        if (sixsRolled == 3 && roll == 6 && this.lastMovedPawn != null) {
            ...
            if (!canMove) {
                Toast.makeText(ComposerCapability.getContext(), "You can't move in this turn",
    ...
    public void announceWinner(JSONArray players, Integer winner) {
        ...
        if (winner == playerIndex)
            ...
            try {
                ...
                throw new ComposerGameException("Error when parsing players list on announceWinner", e);
            }
        }
        this.winnerLabel.setVisibility(View.VISIBLE);
    }
    ...
    private void finishTurn(boolean playersListModified, boolean additionalStep) {
        ...
        if (!additionalStep)
            this.winnerLabel.setVisibility(View.INVISIBLE);
        ComposerCapability.endOfTurn(playersListModified, additionalStep);
    }
